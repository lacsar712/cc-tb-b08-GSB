...
def main():
    ...
    # 老库幂等补列：日账按自然日汇总依赖 created_at
    cur.execute("ALTER TABLE cuppings ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now()")
    # 已签发快照：冻结某日的通过数、不通过数与命中编号集合，签发后一律不动
    cur.execute(
